...
class ClassSentencePath:
    ...
    LAST_PATH_TABLE_WRITE = [0, 0, 0]
    LAST_PATH_TABLE_READ = [0, 0, 0]
    ...
    @staticmethod
    def fresh(table=[0, 0, 0], para=[0, 0]):
        """刷新对象间信息"""
...
class ClassDocxFile:
    ...
    def __fill_table_cell(self, sentence, path):
        ...
        text = cell.text
        if ClassSentencePath.INTERVAL_TABLE:
        ...
        for table in self.PureTemplate.File.tables:
            ...
            for row in table.rows:
                ...
                for cell in row.cells:
                    ...
                    replaces = re.findall(re_test, cell.text)
                    cell.text += "$%&"
                    replaces = list(set(replaces))  # 优化合并同类项
                    if len(replaces) == 0:
                        now_cell += 1
